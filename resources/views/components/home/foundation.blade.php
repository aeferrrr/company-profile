<section class="py-5 bg-light position-relative">
                        ...
                        <div class="swiper-slide">
                            <div class="value-card bg-white p-4 rounded-4 shadow-sm h-100">
                                <div class="d-flex align-items-center mb-4">
                                    ...
                                    <div>
                                        <h5 class="fw-bold mb-0">{{ __('messages.values.excellence_title') }}</h5>
                                        <small class="text-muted">{{ __('messages.values.excellence_sub') }}</small>
                                        <div class="text-primary small mt-1">★★★★★</div>
                                    </div>
                                </div>
                                <p class="text-muted mb-0 small">
                                    "{{ __('messages.values.excellence_desc') }}"
                                </p>
                            </div>
                        </div>
    ...
    <style>
        .value-card { border: 1px solid #f8f9fa; transition: all 0.3s ease; }
        .value-card:hover { transform: translateY(-5px); box-shadow: 0 1rem 3rem rgba(255, 255, 255, 0.075) !important; }
        .icon-circle { width: 60px; height: 60px; border-radius: 50%; display: flex; align-items: center; justify-content: center; }
        .btn-nav-custom { width: 50px; height: 50px; border-radius: 50%; border: none; background-color: #badcf3; color: #333; display: flex; align-items: center; justify-content: center; transition: all 0.3s; cursor: pointer; }
        .btn-nav-custom:hover, .btn-nav-custom.active { background-color: #0734ff; color: #000; box-shadow: 0 4px 10px rgba(255, 193, 7, 0.4); }
    </style>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            var valSwiper = new Swiper(".valueSwiper", {
                ...
                navigation: { nextEl: ".next-val", prevEl: ".prev-val" },
            });
        });
    </script>
</section>